...
class Pedido
{
    ...
    //borra los pedidos que ya no pertenecen a ninguna comanda
    public static function BorrarPedidosComanda() {
        $objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso(); 
        $consulta = $objetoAccesoDato->RetornarConsulta("
        delete from pedidos
        WHERE idComanda not in (SELECT codigo FROM comandas)");
        $consulta->execute();
        return $consulta->rowCount();
    }
    ...
    public static function EntregarPedido($id) {
        ...
        if($pedido->estado == 'listo'){
            ...
            return "El pedido aun no se encuentra listo o ya fue entregado";
        }
    }
	...
	public function Cancelar() {
        if ($this->estado == 'entregado') {
            ...
            if ($this->estado == 'preparandose' || $this->estado == 'pendiente') {
